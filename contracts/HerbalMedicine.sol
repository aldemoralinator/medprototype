...
contract HerbalMedicine {
  ...
  modifier onlyWhenPackageRecieverIsMember( address _packageReciever ) {
    require( 
      accounts[_packageReciever].isMember == true, 
      "You are sending a package to a non-member"
    );
    _;
  }
  ...
  function sendPackage( 
    uint256 _packageKey, 
    address _packageReciever
  ) public 
    onlyMember 
    onlyPackageHandler( _packageKey, _packageReciever )
    onlyWhenPackageRecieverIsMember( _packageReciever ) {
